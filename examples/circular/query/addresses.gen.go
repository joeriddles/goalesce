...
func newAddress(db *gorm.DB, opts ...gen.DOOption) address {
	...
	_address.OccupantID = field.NewInt64(tableName, "occupant_id")
	_address.Occupant = addressBelongsToOccupant{
		db: db.Session(&gorm.Session{}),

		RelationField: field.NewRelation("Occupant", "model.Person"),
		Home: struct {
			field.RelationField
			Occupant struct {
				field.RelationField
			...
			RelationField: field.NewRelation("Occupant.Home", "model.Address"),
			Occupant: struct {
				field.RelationField
			}{
				RelationField: field.NewRelation("Occupant.Home.Occupant", "model.Person"),
			},
		},
	}
...
type address struct {
	...
	ID         field.Int64
	City       field.String
	OccupantID field.Int64
	Occupant   addressBelongsToOccupant
...
func (a *address) updateTableName(table string) *address {
	...
	a.OccupantID = field.NewInt64(table, "occupant_id")
...
func (a *address) fillFieldMap() {
	...
	a.fieldMap["occupant_id"] = a.OccupantID
...
type addressBelongsToOccupant struct {
	db *gorm.DB
...
func (a addressBelongsToOccupant) Where(conds ...field.Expr) *addressBelongsToOccupant {
	if len(conds) == 0 {
		return &a
	}
...
func (a addressBelongsToOccupant) WithContext(ctx context.Context) *addressBelongsToOccupant {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a addressBelongsToOccupant) Session(session *gorm.Session) *addressBelongsToOccupant {
	a.db = a.db.Session(session)
	return &a
}

func (a addressBelongsToOccupant) Model(m *model.Address) *addressBelongsToOccupantTx {
	return &addressBelongsToOccupantTx{a.db.Model(m).Association(a.Name())}
}

type addressBelongsToOccupantTx struct{ tx *gorm.Association }

func (a addressBelongsToOccupantTx) Find() (result *model.Person, err error) {
	return result, a.tx.Find(&result)
}

func (a addressBelongsToOccupantTx) Append(values ...*model.Person) (err error) {
...
func (a addressBelongsToOccupantTx) Replace(values ...*model.Person) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a addressBelongsToOccupantTx) Delete(values ...*model.Person) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a addressBelongsToOccupantTx) Clear() error {
	return a.tx.Clear()
}

func (a addressBelongsToOccupantTx) Count() int64 {
	return a.tx.Count()
}
